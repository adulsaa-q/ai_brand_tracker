...
def migrate_csv_to_stores(csv_path="sample_output/results_sample.csv", duckdb_path="data/intelligence.duckdb", sqlite_path="data/intelligence.db"):
    ...
    with duckdb_store._get_connection() as con_duck:
        for (ts, prompt_text, model), group in grouped:
            ...
            for _, row in group.iterrows():
                ...
                mentioned = bool(row["mentioned"] == 1 or row["mentioned"] is True or str(row["mentioned"]).lower() == "true")
                ...
                con_duck.execute("""
                    ...
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                """, [mention_id, obs_id, ts, query_id, brand_id, mentioned, rank, "recommended" if mentioned and (rank or 1) <= 2 else "neutral_mention", sentiment, strengths_json, weaknesses_json])
    ...
    print(f"🎉 Migration Complete: {obs_count} Observations, {mention_count} Brand Mentions, {citation_count} Citations persisted to DuckDB & SQLite.")
